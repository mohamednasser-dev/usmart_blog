...
<!-- End RTL -->
  <head>
      <title>{{$data['blog']['browser_title']}}</title>
      <meta name="viewport" content="width=device-width, initial-scale=1, shrink-to-fit=no">
      <meta name="description" content="{{$data['blog']['meta_description']}}">
      <meta name="keywords" content="{{$data['blog']['keywords']}}">
  ...
  <body class="red-nav"> <!-- <=============this class handle nav colors -->
    <!-- ============Start nav============ -->
    ...
    <div class="postSlider">
      @foreach ( $data['header_blogs'] as $blog)
      <div>
        <img src="https://res.cloudinary.com/dqthprijd/image/upload/w_500,h_500,c_fill,q_100/v1586068288/{{$blog->image}}" alt="{{$blog->image_alt}}">
        <div class="caption text-center p-3 t-white">
            <a href="/blog/{{$blog->url}}" class="btn btn-danger"> {{$blog->first_title}} </a>
            <h6>{{$blog->second_title}}</h6>
            <small>{{$blog->views}} <i class="fa fa-eye"></i></small>
            {{-- <small>41 <i class="fa fa-comment"></i></small> --}}
            <small class="only_date" >{{$blog->created_at}} <i class="fa fa-calendar"></i></small>
            <small>{{ $blog->written_by }}</small>
        </div>
      </div>      
      @endforeach
    ...
    <div class="container">
       <!-- <h1 class="text-center">يوضع عنوان هنا يوضع عنوان هنا</h1> -->
       <div class="row">
           <div class="col-md-8">
                <!-- start image section -->
                <a target="_blank" href="{{$data['main_ad']->content}}" class="row p-2 wow fadeInUp">
                    <img src="https://res.cloudinary.com/dqthprijd/image/upload/w_1080,h_400,c_fill,q_100/v1586068288/{{$data['main_ad']->image}}" alt="" class="img-fluid">
                </a>
                <!-- end image section -->
                <!-- share section -->
                ...
                <div class="row border p-2 m-1 wow fadeInUp">
                    <div class="media">
                        <img src="https://res.cloudinary.com/dqthprijd/image/upload/w_180,h_180,c_fill,q_100/v1586068288/{{$data['blog']['image']}}" class="align-self-center mr-3" alt="{{$data['blog']['image_alt']}}" width="60">
                        <div class="media-body">
                            <p>{{$data['blog']['first_title']}}
                            </p>
                            <small class="pull-right since-ar">{{$data['blog']['created_at']}}</small>
                        </div>
                    </div>
                </div>
                <div class="post">
                    <h1 class="pt-4 wow fadeInUp">{{$data['blog']['first_title']}}</h1>
                    <h5 class="pt-4 wow fadeInUp">{{$data['blog']['second_title']}}</h5>
                    <img src="https://res.cloudinary.com/dqthprijd/image/upload/w_1080,h_600,c_fill,q_100/v1586068288/{{$data['blog']['image']}}" alt="{{$data['blog']['image_alt']}}" class="img-fluid wow fadeInUp">
                    <div class=" border p-2 mt-3 wow fadeInUp">
                        <h5 class="pt-4">{{$data['blog']['first_title']}}</h5>
                        <ul class="list-unstyled">
                            <li>
                                <a class="t-black" >{{$data['blog']['second_title']}}</a>
                            </li>
                            <li>
                                <a class="t-black" >{{$data['blog']['third_title']}}</a>
                            </li>
                        </ul>
                    </div>
                    ...
                    <div class="mt-5 wow fadeInUp">
                        <span>مشاركة</span>
                        <div class="addthis_inline_share_toolbox"></div>
                        <span class="float-right">مشاهدة {{$data['blog']['views']}}</span>
                    </div>
                </div>
                <hr>
           </div>
           <div class="col-md-4 sidebar">
                <!-- start latest news -->
                <div class="row p-0">
                    <h5 class="underline p-3 wow fadeInLeft">اخر الأخبار</h5>
                    @foreach ( $data['news_blogs'] as $blog)
                    <div class="media pt-2  wow fadeInLeft">
                      <img src="https://res.cloudinary.com/dqthprijd/image/upload/w_180,h_180,c_fill,q_100/v1586068288/{{$blog->image}}" class="mr-3" alt="{{$blog->image_alt}}" width="80">
                      <div class="media-body">
                          <p class="small m-0"> {{$blog->first_title}} </p>
                          <small>{{$blog->views}} <i class="fa fa-eye"></i></small>
                          {{-- <small>41 <i class="fa fa-comment"></i></small> --}}
                          <small class="only_date" >{{$blog->created_at}} <i class="fa fa-calendar"></i></small>
                          <small>{{$blog->written_by}}</small>
                ...
                <!-- feachered news -->
                <div class="row p-3">
                    <h5 class="underline  wow fadeInLeft">مختارات</h5>
                    @foreach ( $data['selects_blog'] as $blog)
                    <article class="wow fadeInLeft">
                        <img src="https://res.cloudinary.com/dqthprijd/image/upload/w_450,h_200,c_fill,q_100/v1586068288/{{$blog->image}}" class="img-fluid" alt="{{$blog->image_alt}}">
                        <div class="media-body">
                            <p class="small m-0">  {{$blog->first_title}} </p>
                            <small>{{$blog->views}} <i class="fa fa-eye"></i></small>
                            {{-- <small>41 <i class="fa fa-comment"></i></small> --}}
                ...
                <div class="row p-3  wow fadeInLeft">
                    <h5 class="underline">الأقسام</h5>
                    <ul class="list-unstyled categories">
                      @foreach ( $data['categories'] as $category )
                        <li><a href="/blog/category/{{$category->url}}">{{$category->title_ar}} ({{$category->count}})</a></li>
                      @endforeach
                    </ul>
                </div>
                <!-- end categories -->
                <!-- feachered news -->
                <div class="row p-3 wow fadeInLeft">
                    <h5 class="underline">افضل المقالات</h5>
                    @foreach ( $data['best_blog'] as $blog)
                    <article>
                        <img src="https://res.cloudinary.com/dqthprijd/image/upload/w_450,h_200,c_fill,q_100/v1586068288/{{$blog->image}}" class="img-fluid" alt="{{$blog->image_alt}}">
                        <div class="media-body">
                            <p class="small m-0"> {{$blog->first_title}} </p>
                ...
                <!-- tags section -->
                <div class="row p-3 wow fadeInLeft">
                    <h5 class="underline">الكلمات اللدلالية</h5>
                    <div>
                      @foreach ($data['blog']['tags_array'] as $tag)
                        <button type="button" class="btn btn-light btn-sm m-1">{{$tag}}</button>
                      @endforeach
                ...
                <!-- start one ads section -->
                @foreach ($data['ads'] as $ad)
                <a target="_blank" href="{{$ad->content}}" class="row pl-3 pr-3 pt-2 pb-2  wow fadeInLeft">
                  <img  src="https://res.cloudinary.com/dqthprijd/image/upload/w_450,h_450,c_fill,q_100/v1586068288/{{$ad->image}}" alt="" class="img-fluid">
                </a>                
                @endforeach
                <!-- end one ads section -->
       ...
       <section class="row pt-5">
        <div class="col-md-12">
        <h4 class="float-left pt-1">مقالات عن {{$data['blog']['category']['title_ar']}}</h4>
          <a href="/blog/category/{{$data['blog']['category']['url']}}" class="btn btn-outline-dark float-right"> عرض الجميع</a>
        </div>
        <div class="row mt-3 p-2 ml-0 mr-0">
          ...
          <a href="/blog/{{$blog->url}}" class="col-md-3 col-sm-6 p-2 wow fadeInUp border text-left t-black">
            <img class="img-fluid" src="https://res.cloudinary.com/dqthprijd/image/upload/w_450,h_200,c_fill,q_100/v1586068288/{{$blog->image}}" alt="{{$blog->image_alt}}">
    ...
    <footer class="mt-5 main-footer t-white">
      ...
      <section class="row copyright t-white p-4 ">
        <div class="col-md-4 text-left">
          <a href="#">
            <img src="/assets/imgs/linkedin.png" alt="linkedin" width="30">
          </a>
          <a href="#">
            <img src="/assets/imgs/twitter.png" alt="linkedin" width="30">
          </a>
          <a href="#">
            <img src="/assets/imgs/linkedin.png" alt="linkedin" width="30">
          </a>
    ...
    <script type="text/javascript" src="//s7.addthis.com/js/300/addthis_widget.js#pubid=ra-5e8c8efe99db9247"></script>
    <script>
      $(".only_date").each(function(){
        var text = $(this).text();
        var post_date = moment(text)
        var since_time = post_date.format('MM/DD/YYYY');
        $(this).html(since_time+" <i class='fa fa-calendar'></i>")
      })
